...
const TimeTable = () => {
  ...
  const getTableData = async (division_name, semester_number) => {
    await axios({
      method: 'post',
      url: 'http://localhost/achievers/get_time_table.php',
      data: {
        "branch": branchName,
        "sem": semester_number,
        "div": division_name
  ...
  const onUpdateData = async (json) => {
    if(json) {
      await axios({
        method: 'post',
        url: 'http://localhost/achievers/set_time_table.php',
        data: { 
          "branch": branchName,
          "sem": semesterNumber,
          "div": divisionName,
      ...
      <Flex className="scrollBar" direction='column' pt={{ base: "120px", md: "75px" }}>
        ...
        <Card>
          <CardHeader p='6px 0px 22px 0px'>
            <Text fontSize='xl' color={textColor} fontWeight='bold'> Time Table </Text>
          </CardHeader>
          <CardBody flexDirection="column">
            ...
            {((designation==="faculty" || designation==="hod") && divisionName && semesterNumber)
              ? <FormControl display={"block"}>
                  {/* <FormLabel ms='4px' fontSize='sm' fontWeight='normal'> Division </FormLabel> */}
